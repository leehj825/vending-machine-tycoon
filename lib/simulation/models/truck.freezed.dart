...
/// Current route: List of machine IDs to visit in order
 ...
 int get pathIndex; Map<Product, int> get inventory;
...
class _Truck extends Truck {
  const _Truck({required this.id, required this.name, this.fuel = 100.0, this.capacity = 100, final  List<String> route = const [], this.currentRouteIndex = 0, this.status = TruckStatus.idle, this.currentX = 0.0, this.currentY = 0.0, this.targetX = 0.0, this.targetY = 0.0, final  List<({double x, double y})> path = const [], this.pathIndex = 0, final  Map<Product, int> inventory = const {}}): _route = route,_path = path,_inventory = inventory,super._();
